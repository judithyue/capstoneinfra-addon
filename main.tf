...
# Create or update a GitHub Actions Repository Variable dynamically
# dynamically create a var AWS_ROLE_TO_ASSUEM in your GitHub repo with the ARN of the role you just created, so your pipeline can use it without hardcoding, on voting app repo, go to Settings > Secrets and Variables > Actions > New Repository Variable
resource "github_actions_variable" "aws_role_to_assume" {
  # The exact name of your Voting App repository on GitHub
  repository = "example-voting-app"

  # The name of the variable inside GitHub Actions
  variable_name = upper("AWS_ROLE_TO_ASSUME_${var.naming_prefix}")

  # Dynamically inputs the ARN generated by your IAM resource above
  value = aws_iam_role.github_actions_role.arn
}
